...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePlan(LogicalDistinct &op) {
	...
	// we add a default hash aggregation in the plan to select the distinct groups
	...
	// we add a default projection on top of the aggregation to project all members in the select list
